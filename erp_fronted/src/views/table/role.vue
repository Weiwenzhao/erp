  ...
  <div class="app-container">
    ...
    >
      <el-table-column
        ...
        width="55"
      />
      <el-table-column label="角色编码" align="center" min-width="180">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="名称" min-width="80px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="roleDetail" label-position="left" label-width="70px" style="width: 80%; margin-left:50px;">
...
export default {
  ...
  methods: {
    getList() {
      ...
      getRoles({ 'pageNo': 1, 'pageSize': 10, 'companyId': '1' }).then(resp => {
        console.log(resp)
        this.list = resp.data
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addRole(this.roleDetail).then((resp) => {
            console.log(resp)
